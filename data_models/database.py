...
class OrcaleDatabase:
    ...
    def preload_base_models(self):
        ...
        try:
            self.__load_linked_words_models()
        except Exception as e:
            print(f"Warning: preload base models - failure: loading linked words models: {e}")
        try:
    ...
    def __load_linked_words_models(self):
        for word in linking_words:
            if self.connection:
                with self.connection.cursor() as cur:
                    cur.execute("SELECT COUNT(*) FROM linked_words WHERE word = :word", [word])
                    print(f"Checking existence of linked word '{word}'...")
                    row = cur.fetchone()
                    if row and row[0] > 0:
                        print(row)
                        print(f"Linked word '{word}' already exists. Skipping insertion.")
                    else:
                        try:
                            cur.execute("INSERT INTO linked_words (word) VALUES (:word)", [word])
                            self.connection.commit()
                        except Exception as e:
                            print(f"Error inserting linked word '{word}': {e}")
    ...
    def __load_tags_models(self):
        ...
        for key in sql_bricks:
            if self.connection:
                with self.connection.cursor() as cur:
                    cur.execute("SELECT COUNT(*) FROM TAGS WHERE name = :name", [key])
                    ...
                    if row and row[0] > 0:
                        ...
                        try:
                            ...
                            print(f"Error inserting tag '{key}': {e}")
        ...
        for file_name in root_model_files:
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                for line_num, line in enumerate(f, start=1):
                    ...
                    with self.connection.cursor() as cur:
                        cur.execute(
                            "INSERT INTO tokens (text_id, token, page, line, position, prev_token_id, next_token_id, tag_id) "
                            "VALUES (:text_id, :token, :page, :line, :position, :prev_token_id, :next_token_id, :tag_id)",
                            [None, token, None, line_num, None, None, None, tag_id]
                        )
        ...
        try:
            with self.connection.cursor() as cur:
                for token_data in token_table_dump:
                    # token_data structure from tokenize_text_content: [text_id, token, page, line, position, tag_id, 0, 0, 0, 0]
                    # We only need: text_id, token, page, line, position, tag_id (first 6 elements)
                    text_id = token_data[0] if len(token_data) > 0 else None
                    token = token_data[1] if len(token_data) > 1 else None
                    page = token_data[2] if len(token_data) > 2 else None
                    line = token_data[3] if len(token_data) > 3 else None
                    position = token_data[4] if len(token_data) > 4 else None
                    tag_id = token_data[5] if len(token_data) > 5 else None

                    cur.execute(
                        "INSERT INTO tokens (text_id, token, page, line, position, tag_id) "
                        "VALUES (:text_id, :token, :page, :line, :position, :tag_id)",
                        [text_id, token, page, line, position, tag_id]
                    )
                self.connection.commit()
        except Exception as e:
